...
`origin/main` the encounter publishes `TurnEndTopic` (`combat.go`) but not yet
`TurnStartTopic` — `seedActorTurn` seeds the economy without publishing it. So
`TurnManager` is currently the **only** live publisher of `TurnStartTopic`, while
four conditions (`Dodging`, `Helped`, `RecklessAttack`, `Unconscious`)
*subscribe* to it. Moving `TurnStart` publishing into the encounter is tracked
under rpg-toolkit#699; once that lands (in Beat 2), `TurnManager` is fully
orphaned and safe to delete, which the post-Beat-2 restructure folds in.
...
a `require` on `rulebooks/dnd5e`. This PR corrects overview.md and rebuilds it
around two diagrams that show the **decided end-state** (encounter inside the
rulebook), with today's separate-module reality annotated "migration pending."
...
- **TurnManager deletion folded in:** by post-Beat-2, rpg-toolkit#699 has moved
  `TurnStart` publishing into the encounter, so the fossil is orphaned and the
  restructure deletes `rulebooks/dnd5e/combat/turn_manager.go` (+ its tests).
...
- rpg-toolkit#699 — moving `TurnStart` publishing into the encounter (orphans the
  fossil so the restructure can delete it).
