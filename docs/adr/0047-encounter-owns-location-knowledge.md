# ADR-0047: Encounter Owns Location Knowledge
...
deltas. Behavior remains a view consumer, session remains a mirror, and Intel
remains an opaque store.
...
exists, even when the available path or budget makes that branch return `Pass`.
It considers the closest reachable remembered player only when no visible
standing player exists. A player newly visible after a remembered-directed step
therefore interrupts remembered pursuit on the next call. This decision adds no
sticky target commitment. Equal-distance remembered subjects use a temporary,
deterministic subject-ID tie-break; richer multi-target sentience remains
deferred.
...
unknown testimony. The correction is based only on the mover's own prior held
Intel, the arrival cell, and the complete lawful percept produced by that
movement refresh. It does not read or reveal the absent subject's concealed
live position. The correction preserves the holding and subject, reports
through the existing Sight channel, and remains `Held` because no channel
