...
This branch was cut before either merged, so it does not carry their plan.md
text yet — same expected small doc-merge-conflict caveat those two already
recorded for each other.
...
precedent this needs a sibling of). Root's `castContent` enablement still
waits on that.
