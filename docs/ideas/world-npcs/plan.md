...
- [x] **Settled (2026-09-02, fourth pass):** add `npcs.NewMerchant(config
  *MerchantConfig) (*Vendor, error)` to `rulebooks/dnd5e/npcs` — real,
  ...
  catalog.
- [x] Draw the explicit boundary with #1275: #1280 owns world placement and
...
- [x] Add top-level `npc` for reusable common-NPC definition/data.
- [x] Name the primary generic struct `NPC`, not `Definition`.
- [x] Define the common NPC facts known now: ref, display name, interaction
  capabilities, combat policy, observation policy, disposition policy, and
  movement policy.
- [x] Use `*core.Ref` for NPC refs.
- [x] Keep the package rule-agnostic: no D&D stats, no D&D combat rules, no D&D
  encounter/session imports.
- [x] Model `npc` as the generic carrier that D&D vendor types can compose with,
  not as the owner of vendor stock or shop behavior.
- [x] Treat package defaults as authoring convenience only; placed/loadable
  encounter state must carry explicit current values.
- [x] Ship only the `vendor` built-in interaction capability for now, while
  keeping the capability shape extensible.
- [x] Keep vendor stock, quote, buy, wallet, and inventory fields out of this
  package.
- [x] Add `ToData`/load or simple data validation, matching nearby package
  conventions.
- [x] Defer `npc/npcs` built-in/common profile registry until implementation or
  #1275 proves it is needed.
- [x] Leave actual D&D vendor type/profile constructors to #1275 under
  `rulebooks/dnd5e/vendors` or the nearest existing D&D content package.

Tests:

- [x] common NPC data round-trips and deep-copies capabilities;
- [x] unknown capability strings remain carried as opaque capability data if the
  final type permits extension;
- [x] NPC refs are `*core.Ref` and survive data round-trip;
- [x] unknown combat, observation, or disposition policies reject by name;
- [x] first vendor profile exposes vendor capability, neutral disposition,
...
- [x] Add `KindWorld MemberKind = "world"` (`field.go`) — no other new type.
- [x] Add the one new validation rule, mirroring the existing player-decider
  check exactly: `KindWorld` + `Decider != nil` → reject with the same
  sentinel the player check uses.
- [x] Do NOT add `Ref`, `Capabilities`, `CombatPolicy`, `ObservationPolicy`,
...
- [x] Do NOT enforce zero `SpeedFeet`/empty `Actions`/empty `Targeting` for
...
- [x] Do NOT touch `memberEntity.BlocksMovement()` — it stays `false`
  unconditionally, for every kind, exactly as before this feature.

Tests:

- [x] setup accepts one player plus one `KindWorld` member on valid floor,
  with the same bare fields (`ID`, `Name`, `Position`) any member needs;
- [x] `Join` (mid-scene) also accepts a `KindWorld` member — it is the only
  mid-scene placement path this package has, shared with monsters (no
  separate `AddMonster` verb exists);
- [x] setup and Join both reject a `KindWorld` member carrying a `Decider`;
- [x] a player or monster is untouched by the new check (no regression on
  the existing player-decider rejection).

## Task 4 - Combat Exclusion in Encounter

- [x] Confirm (do not implement) that `sidesInContactOrder`'s switch already
  ...
  this MVP.
- [x] Confirm (do not implement) `Pump` already skips `KindWorld` — its
  monster loop is filtered to `KindMonster` explicitly.
- [x] Confirm fight formation and straggler joining exclude `KindWorld` —
  both are fed exclusively from `sidesInContactOrder`, so this follows from
  the first bullet.
- [x] Add a defense-in-depth guard on `Transfer`: reject moving a `KindWorld`
...
- [x] Confirm `ClockOf` for a `KindWorld` member reports the world clock —
  automatic, from `e.clock.Join(...)` running unconditionally for every kind
  (see Task 1). Not something to build.

Tests:

- [x] first light with player + monster + `KindWorld` member forms only the
  player/monster fight (proven via `Join`/`Step`'s public `Formed` output,
  not by calling `sidesInContactOrder` directly);
- [x] a monster's `Pump`-driven walk into contact with a `KindWorld` member
  forms no fight;
- [x] `ClockOf` reports the world clock for a `KindWorld` member both before
  and after a real fight forms between a player and a monster elsewhere on
  the map;
- [x] `Transfer` to `ClockTurn` refuses a `KindWorld` member by name;
- [x] an encounter with a `KindWorld` member produces identical trigger
...
- [x] Add `encounter.Interact` (`InteractInput{Actor, Target, Range}` →
  `InteractOutput{Target, Seq}`) — no descriptor, no capabilities, no policy.
- [x] Default range to one cell unless a caller supplies a positive range.
- [x] Require actor to be a player member for MVP.
- [x] Require target to be a `KindWorld` member.
- [x] Require adjacency plus current visibility; do not require an unobstructed
  path or movement-reachable cell beyond the visibility check.
- [x] Append a story beat confirming the interaction; no NPC-state mutation.
- [x] Add `session.WorldNPCDescriptor{TargetID, Ref, DisplayName, Capabilities,
...
- [x] adjacent player can `Interact` and gets back the confirmed target ID;
- [x] adjacent but not visible target is refused;
- [x] visible but non-adjacent target is refused;
- [x] distant player is refused;
- [x] non-player actor is refused;
- [x] monster target is refused;
- [x] interaction with a closed encounter returns `ErrClosed`.

Tests (session-level, added alongside Task 7):

- [x] `session.Interact` returns a `WorldNPCDescriptor` with ref/name/
  capabilities/policy sourced from session's store, for a target `encounter`
  confirmed reachable;
- [x] descriptor capability slice is copy-out.
...
- [x] Confirm a `KindWorld` blob round-trips through `ToData`/`LoadEncounter`
  ...
  applied at load).
- [x] Confirm old player/monster blobs — with no `"world"` kind present at
  all — keep loading unchanged.

Tests:

- [x] an encounter containing a `KindWorld` member round-trips through
  `ToData`/`LoadEncounter`;
- [x] a loaded `KindWorld` member can still be `Interact`-ed with;
- [x] a pre-existing player/monster-only blob loads exactly as it did before
  this feature.
...
- [x] **Filled in (2026-09-02, third pass):** `npc.Data` carries no
...
- [x] **Corrected (2026-09-02, second pass):** the placement verb does NOT
  ...
  written).
- [x] Add `session.Interact` per Task 5, returning:
...
- [x] **Filled in (2026-09-02, third pass):** `TestNoInnerTypeCrossesTheBoundary`
  ...
  package definition.)
- [x] Leave vendor stock, quotes, purchases, and character-inventory mutation to
  #1275; only carry `npc.NPC` identity/capabilities/policy here.
- [x] Do not reuse `SpawnOutput.NPC` or `SessionData.NPCs` for world NPCs (N1).

Tests:

- [x] session can `PlaceNPC` and later start/load an encounter containing it;
- [x] `PlaceNPC` rejects a nil `NPC` field by name;
- [x] session interaction returns a `WorldNPCDescriptor` and save/delivery
  reports;
- [x] session attack-candidate projection excludes `KindWorld` members
  (fixing `buildTargetPreflight` in `offers.go`, which today has no kind gate
  at all — visibility/range only);
- [x] session `Attack` rejects a `KindWorld` target before resolution;
- [x] existing spawned-monster behavior is unchanged — sheets still land in
  `SessionData.NPCs`;
- [x] `TestNoInnerTypeCrossesTheBoundary` passes with the new allowlist
...
- [x] Add one narrative test with a player, a monster, and a vendor-profile
...
- [ ] Add or update an Example if the final API has a clean public interaction
  story worth pinning. **Not done** — the acceptance test (#1415) covers the
  narrative; no dedicated `Example` function was added. Optional, low
  priority, not blocking.
- [ ] Update package docs/README text only where current behavior changes.
  **Not done** — `field.go`'s `MemberKind` doc and `memberEntity`'s doc were
  updated inline as part of the relevant PRs, but `encounter`'s own package
  doc.go was not revisited to mention `KindWorld`. Worth a small follow-up
  if someone reads that file expecting it to be current.

Verification:

- [x] `go test -race ./...` from `rulebooks/dnd5e/encounter`;
- [x] `go test -race ./...` from `rulebooks/dnd5e/session`;
- [x] `golangci-lint run ./...` from each changed module, if available;
- [x] root `git diff --check`;
- [x] no committed local `replace` directives or `go.work`.

## Post-Merge

- [x] Add `implementation.md` with final commit/tag evidence, deviations, and
  observed behavior.
- [x] Comment on #1280 with the implementation PR and verification evidence.
- [x] Comment on #1275 with the world-NPC framework version or PR it should
  build against.
- [ ] Open follow-up issues for dialogue, quests, trainers, attackable NPC
  ...
  **Deliberately not done** — none of these have a real consumer yet, and
  opening speculative issues for them now would be exactly the kind of
  premature scope this whole effort has repeatedly cut in favor of the
  smallest working slice. Open one when something actually needs it.
