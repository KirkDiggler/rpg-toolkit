...
- **Arcade recovery retired; normal long-rest rules are authoritative (rpg-toolkit#1365).** The published root D&D rules provide `Character.LongRest` as the single recovery meaning; the parallel persisted-data launch reset has been removed. First-admission orchestration is not implemented here: `resolution` and `session` will supply it in separate tagged PRs. Existing consumers remain on their pinned provider until they deliberately advance. See [encounter.md](architecture/components/encounter.md#cross-encounter-recovery-arcade-recovery-retired).
