...
- **Equip rules for the wire: two-handed occupancy + `EquipmentView` as the single display projection (rpg-toolkit#811, PR #812, 2026-07-21, tagged rulebooks/dnd5e/v0.67.0).** `Character.EquipItem` gains real occupancy — previously a bare `equipmentSlots.Set`: a two-handed weapon claims main hand and clears off hand, equipping off hand while a two-hander is held frees main hand, an incompatible slot now errors instead of silently succeeding, and the vacate-old-slot pass runs unconditionally before either occupancy branch (not just guarding the non-two-handed path — `LoadFromData` copies `EquipmentSlots` verbatim with zero validation, so corrupted/legacy state could otherwise leave an item duplicated across two slots; a Copilot #812 catch, traced and fixed before merge). The compatibility rule is exported as `CompatibleSlots(item) []InventorySlot`, the single source of truth both `EquipItem` and the new `EquipmentView` build on. `Character.EquipmentView(ctx) *EquipmentView` is the slice's real design point: the ONE projection rpg-api maps field-for-field to `CharacterData`'s equipment surface (`Items[]{Name,Kind,SlotKeys,Slot,StatLine}`, a `Slots` taxonomy, `ACTotal`, `ACNote`, `MainHandDamage`) — composing nothing itself. `ACNote` derives generically from `ACBreakdown.Components`' `Type`/`Source` (no per-feature hardcoding — required giving `EffectiveAC`'s DEX components a real `Source` ref, previously `nil`); `MainHandDamage` steps a versatile weapon's die up one notch only when the off hand is free, folds in a dual-wield fragment otherwise. Serves the approved rpg-dnd5e-web#557 concept via rpg-api-protos#187 and rpg-api#682/#680. See [rulebook-dnd5e.md](architecture/components/rulebook-dnd5e.md#equip-rules-equipmentview-as-the-single-wire-projection-rpg-toolkit811).
