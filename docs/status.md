...
- **Arcade recovery retired; first-admission normal LongRest implemented (rpg-toolkit#1365, #1396).** The published root D&D rules provide `Character.LongRest` as the single recovery meaning, the published resolution provider exposes the strict record-in/record-out operation, and Session now persists that rest on a character's first-ever admission before placement. The parallel persisted-data launch reset is gone. The thin rpg-api consumer that removes its old launch reset and adopts the new Session release is still pending; no Session tag is named here because CI mints it only after this change merges. Existing consumers remain on their pinned providers until they deliberately advance. See [encounter.md](architecture/components/encounter.md#cross-encounter-recovery-arcade-recovery-retired).
