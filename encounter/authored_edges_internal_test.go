...
// TestCanonicalGeneratedEdgeRecordsWithOverlay_RejectsConnectorDoorCollision
// keeps connector-derived crossings outside the authored replacement rule.
func TestCanonicalGeneratedEdgeRecordsWithOverlay_RejectsConnectorDoorCollision(t *testing.T) {
	from := core.Hex{Q: 0, R: 0, S: 0}
	to := core.Hex{Q: 1, R: -1, S: 0}
	space := &SpaceData{
		AuthoredEdges: []AuthoredEdge{{From: from, To: to, Kind: GeneratedEdgeKindSolid}},
		Regions:       []RegionData{{ID: "semantic-floor", Hexes: core.NewHexSet(to)}},
	}
	enc := &Encounter{data: &Data{
		Space: space,
		Doors: map[core.EntityID]*DoorData{
			"connector-door": {ID: "connector-door", Position: from},
		},
	}}
	_, err := enc.canonicalGeneratedEdgeRecordsWithOverlay(authoredEdgesByKey(space))
	require.Error(t, err)
