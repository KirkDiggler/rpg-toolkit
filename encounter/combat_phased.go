...
//     ReactionTriggerTopic, calls the resolver's ResolveStrikeHit, then
...
//   - Encounter.CompleteStrikeAction runs phase 2 with the reactor responses
...
// TakeStrikePhased dispatches a player attack via the discrete two-phase
...
//     CompleteStrikeAction once all reactors respond.
//
...
func (e *Encounter) TakeStrikePhased(
	...
	if e.strikeResolver == nil {
		return nil, ErrNoStrikeResolver
	}
	...
	// implement StrikeResolver fall back to the legacy single-phase flow.
	phased, ok := e.strikeResolver.(PhasedStrikeResolver)
	if !ok {
		outcome, err := e.strikeResolver.ResolveStrike(input)
		if err != nil {
	...
	// CompleteStrikeAction. NO outcome events are published yet; the encounter
...
// reaction prompt. Called by the orchestrator after TakeStrikePhased
...
// PhasedStrikeContext (opaque to the SDK; the orchestrator marshals it via
...
// to look up the in-flight phase-1 state to feed into CompleteStrikeAction.
...
// Called by the orchestrator after CompleteStrikeAction completes (success
...
// CompleteStrikeAction runs phase 2 of a previously-paused two-phase attack
// with the reactor responses baked in. The orchestrator passes the persisted
// PhasedStrikeContext (from a prior TakeStrikePhased call) and the list of
...
// original 2.11d shape, dispatched via TakeStrikePhased) and monster→player
...
// PhasedStrikeContext through unchanged for either direction. Player→
// player attacks return ErrUnsupportedAttackDirection.
func (e *Encounter) CompleteStrikeAction(attackCtx *PhasedStrikeContext, modifiers []ReactionModifier) error {
	if attackCtx == nil {
	...
	// Resolve attacker against both maps; CompleteStrikeAction is the only
	...
	case attackerPlayer != nil && targetMonster != nil:
		...
		// PhasedStrikeContext, so a resumed attack reports the attack defaults.
...
//     resolver returns.
//   - CompleteStrikeAction (NPC→player Shield-resume direction) — publishes
...
func (e *Encounter) applyAndPublishNPCOutcome(monster *MonsterData, player *PlayerData, outcome *StrikeOutcome) error {
	hpBefore := player.HP
	if outcome.Hit {
	...
	if _, err := e.publishStrikeOutcome(
...
// path and the phased CompleteStrikeAction path (player→monster direction).
...
// CompleteStrikeAction resume path, which lacks the live economy diff, passes the
// attack defaults.
func (e *Encounter) applyAndPublishOutcome(
	player *PlayerData, monster *MonsterData, outcome *StrikeOutcome,
	...
	corrID, err := e.publishStrikeOutcome(
