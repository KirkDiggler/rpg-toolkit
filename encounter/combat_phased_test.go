...
//
// These tests exercise Encounter.TakeStrikePhased + CompleteStrikeAction with
// a stubbed PhasedStrikeResolver. The stub controls whether ReactionTrigger
// events are published during phase 1 and whether the reactor is a player
// (surface) or NPC (auto-resolve).
//
// Wave 2.11e — NPC-attacker CompleteStrikeAction symmetry tests appended at
...
// resume calls CompleteStrikeAction with the NPC as AttackerID.
...
// stubPhasedResolver implements PhasedStrikeResolver and records calls.
// It also lets the test publish ReactionTriggerEvents during ResolveStrikeHit
// so the encounter SDK's buffered subscriber sees them.
type stubPhasedResolver struct {
	hitCalls     []tkenc.StrikeInput
	outcomeCalls []*tkenc.PhasedStrikeContext

	// publishOnHit, when set, is called during ResolveStrikeHit to simulate
	// post-roll subscribers publishing ReactionTriggerEvents on the bus.
	publishOnHit func(bus dnd5events.EventBus)

	// resolverTriggers are returned directly from ResolveStrikeHit in
	...
	hitReturn     *tkenc.PhasedStrikeContext
	outcomeReturn *tkenc.StrikeOutcome
}

func (s *stubPhasedResolver) ResolveStrike(_ tkenc.StrikeInput) (*tkenc.StrikeOutcome, error) {
...
func (s *stubPhasedResolver) ResolveStrikeHit(
	input tkenc.StrikeInput,
) (*tkenc.PhasedStrikeContext, []tkenc.ReactionTrigger, error) {
	s.hitCalls = append(s.hitCalls, input)
	if s.publishOnHit != nil && input.EventBus != nil {
		s.publishOnHit(input.EventBus)
	}
	if s.hitReturn == nil {
		s.hitReturn = &tkenc.PhasedStrikeContext{
			Rulebook:   "stub",
...
func (s *stubPhasedResolver) ApplyStrikeOutcome(
	ctx *tkenc.PhasedStrikeContext, _ []tkenc.ReactionModifier,
) (*tkenc.StrikeOutcome, error) {
	s.outcomeCalls = append(s.outcomeCalls, ctx)
	if s.outcomeReturn == nil {
		s.outcomeReturn = &tkenc.StrikeOutcome{
			Hit:         true,
			AttackRoll:  14,
			AttackBonus: 3,
			TargetAC:    14,
...
func (s *PhasedTakeActionSuite) SetupTest() {
	...
	s.enc = tkenc.New(context.Background(), "enc-1", s.broker, tkenc.WithStrikeResolver(s.resolver))
	...
	// rpg-toolkit#864: TakeStrikePhased now gates melee attacks on reach (a
...
func (s *PhasedTakeActionSuite) TestNoTriggers_ResolvedInline() {
	s.makeAliceActive()
	out, err := s.enc.TakeStrikePhased(alicePlayerID, attackRef(),
...
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_RunsPhase2WithModifiers() {
	s.makeAliceActive()
	s.resolver.publishOnHit = func(bus dnd5events.EventBus) {
		topic := dnd5eEvents.ReactionTriggerTopic.On(bus)
		_ = topic.Publish(context.Background(), dnd5eEvents.ReactionTriggerEvent{
	...
	err = s.enc.CompleteStrikeAction(out.AttackContext, []tkenc.ReactionModifier{
		{ConditionRef: shieldRefStr, ACBonus: 5},
	})
	s.Require().NoError(err)
	s.Len(s.resolver.outcomeCalls, 1, "phase 2 should run once via CompleteStrikeAction")
}

// stubLegacyResolver only implements StrikeResolver (no phased path). Verifies
// TakeStrikePhased gracefully falls back to the monolithic single-phase path.
type stubLegacyResolver struct {
	calls []tkenc.StrikeInput
}

func (s *stubLegacyResolver) ResolveStrike(input tkenc.StrikeInput) (*tkenc.StrikeOutcome, error) {
...
func (s *PhasedTakeActionSuite) TestLegacyResolverFallback() {
	...
	enc := tkenc.New(context.Background(), "enc-2", broker, tkenc.WithStrikeResolver(resolver))
	...
	out, err := enc.TakeStrikePhased(alicePlayerID, attackRef(),
		tkenc.ActionTarget{EntityID: gobEntityID})
	s.Require().NoError(err)
	s.Require().NotNil(out)
...
// --- Wave 2.11e — NPC-attacker CompleteStrikeAction symmetry ---
//
// The goblin attacks bob; bob's Shield prompt fires; SubmitCheck resumes
// via CompleteStrikeAction with AttackerID=gobEntityID, TargetID=bobEntityID.
...
// builds the PhasedStrikeContext directly (bypassing NPCAct since
// CompleteStrikeAction is the surface under test).

// goblinAttackBobContext builds the PhasedStrikeContext that the
// orchestrator would persist when an NPC attack pauses for a player
// reaction. Direction: monster (goblin) → player (bob).
func goblinAttackBobContext() *tkenc.PhasedStrikeContext {
	return &tkenc.PhasedStrikeContext{
		Rulebook:   "stub",
...
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_NPCAttacker_HitPublishes() {
	bobSub, err := s.broker.Subscribe(s.enc.ID(), bobPlayerID)
	s.Require().NoError(err)
	defer func() { _ = bobSub.Close() }()

	s.resolver.outcomeReturn = &tkenc.StrikeOutcome{
		Hit:         true,
		AttackRoll:  16,
		AttackBonus: 4,
		TargetAC:    12,
	...
	err = s.enc.CompleteStrikeAction(goblinAttackBobContext(), nil)
...
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_NPCAttacker_MissNoPublish() {
	bobSub, err := s.broker.Subscribe(s.enc.ID(), bobPlayerID)
	s.Require().NoError(err)
	defer func() { _ = bobSub.Close() }()
...
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_NPCAttacker_PlayerDeath() {
	bobSub, err := s.broker.Subscribe(s.enc.ID(), bobPlayerID)
	s.Require().NoError(err)
	defer func() { _ = bobSub.Close() }()
...
// TestCompleteStrikeAction_NPCAttacker_DamageSharesAttackCorrelationID is a
...
// player-attacker path (Invariant 8). publishStrikeOutcome already stamps
...
// publishStrikeOutcome), so this test passes without any production change;
// it exists to lock the behavior in and document that #723's "NPC attack
// path" symptom does not reproduce here.
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_NPCAttacker_DamageSharesAttackCorrelationID() {
	bobSub, err := s.broker.Subscribe(s.enc.ID(), bobPlayerID)
	s.Require().NoError(err)
	defer func() { _ = bobSub.Close() }()
...
// dispatch in CompleteStrikeAction must reject the resume rather than
...
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_AmbiguousAttacker_Rejected() {
	...
	err := s.enc.CompleteStrikeAction(&tkenc.PhasedStrikeContext{
		Rulebook:   "stub",
		AttackerID: gobEntityID,
		TargetID:   bobEntityID,
	}, nil)
...
func (s *PhasedTakeActionSuite) TestCompleteStrikeAction_AmbiguousTarget_Rejected() {
	s.addCollidingPlayerOnGoblinID()
