...
// ErrNoStrikeResolver is returned by combat verbs when the encounter was
// constructed without a StrikeResolver. The orchestrator (rpg-api) wires
// one via WithStrikeResolver; tests stub it. Without one, the encounter
// SDK has no way to evaluate attack mechanics — it does not embed
// rulebook logic itself.
var ErrNoStrikeResolver = errors.New("no combat resolver wired")

// StrikeResolver bridges the encounter SDK to a rulebook implementation
...
//
// Wave 2.11a wired ResolveStrike for the player-attack path (TakeAction).
...
// relied on it must wire a resolver via WithStrikeResolver.
type StrikeResolver interface {
	// ResolveStrike evaluates one attack from attacker to target and
	...
	// returned StrikeOutcome.
	ResolveStrike(input StrikeInput) (*StrikeOutcome, error)
}

// PhasedStrikeResolver extends StrikeResolver with discrete RPC-phase verbs
// for two-phase attack resolution. The encounter SDK uses these verbs when
// an attack triggers reactions (Wave 2.11d).
//
// PhasedStrikeResolver is an OPTIONAL interface — resolvers that implement
// it enable two-phase + reaction-prompt flows. Resolvers that only implement
// StrikeResolver continue to work for single-phase attacks.
//
// The encounter SDK type-asserts StrikeResolver to PhasedStrikeResolver in
// TakeAction; absence of the phased interface falls back to the monolithic
// ResolveStrike call (today's behavior).
type PhasedStrikeResolver interface {
	StrikeResolver

	// ResolveStrikeHit runs phase 1 of an attack: rolls the d20, evaluates
	// hit/miss against the target's original AC, and returns an opaque
	// PhasedStrikeContext that the orchestrator stores between the two phases.
	//
	...
	ResolveStrikeHit(input StrikeInput) (*PhasedStrikeContext, []ReactionTrigger, error)

	// ApplyStrikeOutcome runs phase 2 of an attack: takes the PhasedStrikeContext
	...
	ApplyStrikeOutcome(ctx *PhasedStrikeContext, modifiers []ReactionModifier) (*StrikeOutcome, error)
}

// PhasedStrikeContext is the opaque state carried between phase 1 and phase 2
...
// it back to ApplyStrikeOutcome when the reactor responds.
//
...
type PhasedStrikeContext struct {
...
// CompleteStrikeAction; the encounter SDK forwards them to ApplyStrikeOutcome.
...
//     calls CompleteStrikeAction with the reactors' choices.
//
...
type TakeActionOutcome struct {
	...
	// stream and call CompleteStrikeAction once the reactors respond.
	...
	AttackContext *PhasedStrikeContext
}

// StrikeInput is the encounter-SDK-side request shape for ResolveStrike.
// The orchestrator's resolver implementation translates this to the
// rulebook's native StrikeInput shape (e.g., dnd5e/combat.StrikeInput).
type StrikeInput struct {
	...
	// rpg-api's Dnd5eStrikeResolver MUST use this bus when non-nil.
...
// StrikeOutcome is the encounter-SDK-side result shape from ResolveStrike.
