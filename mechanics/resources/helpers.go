	...
	for level := 1; level <= 9; level++ {
		if maxSlots, exists := slots[level]; exists && maxSlots > 0 {
			resource := NewSimpleResource(SimpleResourceConfig{
				...
				Owner:           owner,
				Key:             core.MustNewRef(fmt.Sprintf("spell_slots_%d", level), "dnd5e", "resource"),
				Current:         maxSlots,
				Maximum:         maxSlots,
...
func CreateAbilityUse(owner core.Entity, abilityName string, maxUses int, restoreType RestorationType) Resource {
	...
	return NewSimpleResource(SimpleResourceConfig{
		...
		Owner:            owner,
		Key:              core.MustNewRef(fmt.Sprintf("%s_uses", abilityName), "dnd5e", "resource"),
		Current:          maxUses,
		Maximum:          maxUses,
...
func CreateHitDice(owner core.Entity, hitDieType string, level int) Resource {
	return NewSimpleResource(SimpleResourceConfig{
		...
		Owner:           owner,
		Key:             core.MustNewRef(fmt.Sprintf("hit_dice_%s", hitDieType), "dnd5e", "resource"),
		Current:         level,
		Maximum:         level,
		RestoreType:     RestoreLongRest,
...
func CreateActionEconomy(owner core.Entity) []Resource {
	resources := []Resource{
		...
		NewSimpleResource(SimpleResourceConfig{
			...
			Owner:       owner,
			Key:         core.MustNewRef("action", "dnd5e", "action"),
			Current:     1,
			Maximum:     1,
		...
		NewSimpleResource(SimpleResourceConfig{
			...
			Owner:       owner,
			Key:         core.MustNewRef("bonus_action", "dnd5e", "action"),
			Current:     1,
			Maximum:     1,
			RestoreType: RestoreTurn,
		}),
		...
		NewSimpleResource(SimpleResourceConfig{
			...
			Owner:       owner,
			Key:         core.MustNewRef("reaction", "dnd5e", "action"),
			Current:     1,
			Maximum:     1,
			RestoreType: RestoreTurn,
		}),
...
func CreateRageUses(owner core.Entity, level int) Resource {
	...
	if uses == -1 {
		return NewSimpleResource(SimpleResourceConfig{
			...
			Type:        ResourceTypeAbilityUse,
			Owner:       owner,
			Key:         core.MustNewRef("rage_uses", "dnd5e", "resource"),
...
func CreateKiPoints(owner core.Entity, level int) Resource {
	...
	return NewSimpleResource(SimpleResourceConfig{
		...
		Type:             ResourceTypeAbilityUse,
		Owner:            owner,
		Key:              core.MustNewRef("ki_points", "dnd5e", "resource"),
		Current:          level,
		Maximum:          level,
