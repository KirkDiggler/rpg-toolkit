...
func (s *FighterTestSuite) TestFighterEquipmentAsChoices() {
	...
	s.Run("martial weapon choice uses category options", func() {
		...
		for i := range equipChoices {
			if equipChoices[i].ID == choices.ChoiceID("fighter-primary-weapon") {
	...
	s.Run("shield or second weapon choice", func() {
		...
		for i := range equipChoices {
			if equipChoices[i].ID == choices.ChoiceID("fighter-secondary-equipment") {
...
func (s *FighterTestSuite) TestFighterMartialWeaponChoice() {
	// Get all Fighter choices and find the primary weapon choice
	allChoices := fighter.GetEquipmentChoicesAsChoices()
	var weaponChoice *choices.Choice
	for i := range allChoices {
		if allChoices[i].ID == choices.ChoiceID("fighter-primary-weapon") {
			weaponChoice = &allChoices[i]
			break
		}
	}
	s.Require().NotNil(weaponChoice, "should find primary weapon choice")

	s.Run("primary weapon choice expands to categories", func() {
		s.Equal(choices.ChoiceID("fighter-primary-weapon"), weaponChoice.ID)
