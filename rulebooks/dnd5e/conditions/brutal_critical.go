...
func (b *BrutalCriticalCondition) onDamageChain(
	...
	primary := primaryWeaponComponent(event)
	if primary == nil {
		return c, nil
	}
	dieSize, err := parseDieSize(primary.Dice)
	if err != nil {
		return c, rpgerr.Wrapf(err, "failed to parse weapon damage notation: %s", primary.Dice)
	}
