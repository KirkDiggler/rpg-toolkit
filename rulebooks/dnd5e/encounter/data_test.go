...
func (s *DataTestSuite) TestGoldenJSONRich() {
	...
	// The log carries TWO sighting beats at seqs 2 and 3, one per observer
	// and audienced to that observer alone — g1 gains p1, p1 gains g1 — landing
	...
	//
	// Both payloads now carry "down":false (rpg-toolkit#1697): standing is
...
func (s *DataTestSuite) TestActionViewUsesRangeFeetJSONWithoutLegacyAlias() {
	...
	s.Require().NoError(err)
	s.JSONEq(`{"ref":{"module":"dnd5e","type":"weapons","id":"shortbow"},"range_feet":320}`, string(raw))
