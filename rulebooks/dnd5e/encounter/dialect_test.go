...
func (s *DialectSuite) closedBlob() encounter.EncounterData {
	...
	s.Require().Len(data.Intel.Intel.Holdings, 3, "each of them is in plain sight of the other two")
	return data
}
...
func (s *DialectSuite) load(data encounter.EncounterData) error {
	_, err := encounter.LoadEncounter(&encounter.LoadEncounterInput{
		Data: data, Sight: everyoneSeesTheWholeMap{}, Equipment: noHandsAreObserved{}, Standing: everyoneStanding{}, Initiative: orderAsGiven{}, TurnDriver: passDriver{}, Striker: passStriker{}, Mover: quietMover{}, Announcer: quietAnnouncer{},
	})
	return err
}
...
func (s *DialectSuite) TestARoomBearingSightPayloadIsRefused() {
	...
	for _, payload := range []string{
		...
		s.Run(payload, func() {
			data := s.closedBlob()
			holding := data.Intel.Intel.Holdings[core.EntityID("alice")]["bob"]
			holding.Payload = []byte(payload)
			data.Intel.Intel.Holdings[core.EntityID("alice")]["bob"] = holding
