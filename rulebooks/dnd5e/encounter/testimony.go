...
func (e *Encounter) correctArrivedLocations(
	...
	for _, holding := range holdings {
		...
		testimony, ok := DecodeSightTestimony(holding.Payload)
		if !ok || testimony.State != LocationKnown || testimony.Position != arrival {
			continue
		}
	...
	unknown, err := EncodeSightTestimony(SightTestimony{State: LocationUnknown})
	if err != nil {
