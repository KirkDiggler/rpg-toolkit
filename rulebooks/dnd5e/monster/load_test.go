...
// TestARefLessConditionIsRefusedAtTheDoor is the monster twin of the character
// keeper's test of the same name, and the pair is the point: one door each,
// checked once, so neither removal path has to.
//
...
// rather than returning an error. Refusing at admission means it never gets
// that far, which is why onConditionRemoved can call Ref().String() bare.
//
// The fake is deliberately a LIAR rather than merely incomplete: its ToJSON
// embeds a valid ref, so it is well-formed by every measure except the one the
// keeper depends on.
