...
func TestByRef_EveryCanonicalMonsterResolves(t *testing.T) {
	// The 13 refs verified to have a real constructor today (see the
	...
	want := []string{
		...
		refs.Monsters.GoblinBoss().String(),
		refs.Monsters.AnimatedArmor().String(),
	}

	// Closes the "next entry never gets mapping-checked" drift window: a
