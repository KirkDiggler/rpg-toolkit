...
func areaTargetKind(area *combatActions.CastArea) TargetKind {
	if area != nil && (area.Footprint.Shape == combatActions.AreaTriangle ||
		area.Footprint.Origin == combatActions.AreaOriginCasterEdge || area.Footprint.Origin == combatActions.AreaOriginPoint) {
		return TargetCell
	}
	return TargetArea
}
