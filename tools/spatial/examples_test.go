...
func TestEventIntegration(t *testing.T) {
	...
	eventBus.SubscribeFunc(spatial.EventEntityMoved, 0, func(ctx context.Context, event events.Event) error {
		capturedEvents = append(capturedEvents, event)
		t.Logf("Entity moved: %s", event.Source().GetID())
		return nil
	})
	
	eventBus.SubscribeFunc(spatial.EventEntityRemoved, 0, func(ctx context.Context, event events.Event) error {
		capturedEvents = append(capturedEvents, event)
