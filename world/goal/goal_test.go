...
// stoppedClock is a goal.Clock that reports whatever moment it was last told
// to. Time does not pass here; it is set. A deadline test that waited for a
// real second to elapse would be testing the machine it runs on.
type stoppedClock struct{ now time.Time }

func (c *stoppedClock) Now() time.Time { return c.now }

func (c *stoppedClock) Set(now time.Time) { c.now = now }
...
func (s *GoalSuite) SetupTest() {
	...
	s.clock = &stoppedClock{now: deadline.Add(-24 * time.Hour)}
}
